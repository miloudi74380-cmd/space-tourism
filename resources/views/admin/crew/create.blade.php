            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.crew.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700">Image du membre d'équipage</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="bio_fr" id="bio_fr" rows="6" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('bio_fr') border-red-500 @enderror">{{ old('bio_fr') }}</textarea>
                                @error('bio_fr')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="bio_en" id="bio_en" rows="6" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('bio_en') border-red-500 @enderror">{{ old('bio_en') }}</textarea>
                                @error('bio_en')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
